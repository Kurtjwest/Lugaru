    ...
    , brightness(_brightness)
{
    ...
    texcoords[0][0] = (placex - position.x) / size / 2.f + .5f;
    texcoords[0][1] = (placez - position.z) / size / 2.f + .5f;

    vertex[0].x = placex;
    vertex[0].z = placez;
    vertex[0].y = terrain.heightmap[whichx + 1][whichy] * terrain.scale + .01f;
    ...
    texcoords[1][0] = (placex - position.x) / size / 2.f + .5f;
    texcoords[1][1] = (placez - position.z) / size / 2.f + .5f;

    vertex[1].x = placex;
    vertex[1].z = placez;
    if (first) {
        vertex[1].y = terrain.heightmap[whichx + 1][whichy + 1] * terrain.scale + .01f;
    } else {
        vertex[1].y = terrain.heightmap[whichx][whichy] * terrain.scale + .01f;
    }

    placex = (float)whichx * terrain.scale;
    placez = (float)whichy * terrain.scale + terrain.scale;

    texcoords[2][0] = (placex - position.x) / size / 2.f + .5f;
    texcoords[2][1] = (placez - position.z) / size / 2.f + .5f;
    ...
    if (rotation) {
        for (int i = 0; i < 3; i++) {
            rot.y = 0;
            rot.x = texcoords[i][0] - .5f;
            rot.z = texcoords[i][1] - .5f;
            rot = DoRotation(rot, 0, -rotation, 0);
            texcoords[i][0] = rot.x + .5f;
            texcoords[i][1] = rot.z + .5f;
        }
    }
}
    ...
    , brightness(0)
{
    ...
    if (which == 0) {
        ...
        texcoords[0][0] = (placex - position.y) / (size) / 2.f + .5f;
        texcoords[0][1] = (placez - position.z) / (size) / 2.f + .5f;
        ...
        texcoords[1][0] = (placex - position.y) / (size) / 2.f + .5f;
        texcoords[1][1] = (placez - position.z) / (size) / 2.f + .5f;
        ...
        texcoords[2][0] = (placex - position.y) / (size) / 2.f + .5f;
        texcoords[2][1] = (placez - position.z) / (size) / 2.f + .5f;
        ...
        texcoords[0][0] = (placex - position.x) / (size) / 2.f + .5f;
        texcoords[0][1] = (placez - position.y) / (size) / 2.f + .5f;
        ...
        texcoords[1][0] = (placex - position.x) / (size) / 2.f + .5f;
        texcoords[1][1] = (placez - position.y) / (size) / 2.f + .5f;
        ...
        texcoords[2][0] = (placex - position.x) / (size) / 2.f + .5f;
        texcoords[2][1] = (placez - position.y) / (size) / 2.f + .5f;
